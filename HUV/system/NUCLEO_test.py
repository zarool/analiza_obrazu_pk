...
for device in device_list:
    ...
    if (device.vid == NUCLEO_VID) and (device.pid == NUCLEO_PID):
        ...
        try:
            NUCLEOUsbAddress = serial.Serial(port=port_USB, baudrate=115200, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, bytesize=serial.EIGHTBITS, timeout=0.1, xonxoff=False, rtscts=False, write_timeout=None, dsrdtr=True, inter_byte_timeout=None, exclusive=None) # w razie problemów z komunikacją podmień na 0  
            connection = True
...
while connection:
    ...
    if NUCLEOUsbAddress.inWaiting():
        ...
        try: # przyjecie ramki z NUCLEO i dekodowanie.  format:   type:value1:value2:value3
            x = NUCLEOUsbAddress.readline().decode('utf-8')
            print(x)
            try:
